...
#print(os.getcwd())
gpu_id = args.gpu_id
basedir = os.path.dirname(args.config_path)
options = f"{args.config_path}"
...
command = "nvidia-docker run --name {} --ipc=host\
        -v /dev/log:/home/haakohu/DeepPrivacy/log -u 1174424 -v {}:/workspace -v /raid/userdata/haakohu/deep_privacy/data:/workspace/data \
          ...
          -it  haakohu/pytorch python -m deep_privacy.metrics.fid_official.calculate_fid {}".format(
            docker_container,
            filedir,
            gpu_id,
            options
...
print(options)
os.system(command)
cmd = f"nvidia-docker run --rm -it -u 1174424 -v $PWD:/workspace -e CUDA_VISIBLE_DEVICES={args.gpu_id} nvcr.io/nvidia/tensorflow:19.06-py3 python deep_privacy/metrics/fid_official/calculate_fid_official.py models/{model_name}/fid_images/real/ models/{model_name}/fid_images/fake"
os.system(cmd)