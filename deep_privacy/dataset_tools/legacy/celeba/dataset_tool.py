...
from deep_privacy.dataset_tool import utils
import math
import numpy as np
...
def anonymize_image_and_save(imname, x0, y0, width, height):
    try:
        ...
        x0_exp, y0_exp, x1_exp, y1_exp = utils.expand_bbox(
            np.array([x0, y0, x0+width, y0+height]), im.shape, False,
            expansion_factor1=0.25)
        width_exp = x1_exp - x0_exp
        height_exp = y1_exp - y0_exp
